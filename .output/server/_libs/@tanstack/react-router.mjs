import { i as __require, s as __toESM, t as __commonJSMin } from "../../_rolldown.mjs";
import { l as require_react_dom, u as require_react } from "../@floating-ui/react-dom.mjs";
import { r as parseHref, t as createBrowserHistory } from "./history.mjs";
...
function deepEqual(a, b, opts) {
	...
	if (Array.isArray(a) && Array.isArray(b)) {
		...
		for (let i = 0, l$1 = a.length; i < l$1; i++) if (!deepEqual(a[i], b[i], opts)) return false;
		return true;
	}
...
function invariant(condition, message) {
	if (condition) return;
	if (isProduction$1) throw new Error(prefix$2);
...
function parseSegment(path, start, output = new Uint16Array(6)) {
	...
	if (optionalParamBracesMatch) {
		const prefix$3 = optionalParamBracesMatch[1];
		const paramName = optionalParamBracesMatch[2];
		const suffix$2 = optionalParamBracesMatch[3];
		const pLength = prefix$3.length;
		output[0] = 3;
		output[1] = start + pLength;
		output[2] = start + pLength + 3;
		output[3] = start + pLength + 3 + paramName.length;
		output[4] = end - suffix$2.length;
		output[5] = end;
		return output;
	}
	const paramBracesMatch = part.match(PARAM_W_CURLY_BRACES_RE);
	if (paramBracesMatch) {
		const prefix$3 = paramBracesMatch[1];
		const paramName = paramBracesMatch[2];
		const suffix$2 = paramBracesMatch[3];
		const pLength = prefix$3.length;
		output[0] = 1;
		output[1] = start + pLength;
		...
		while (cursor < length) {
			...
			switch (segment[0]) {
				...
				case 1: {
					...
					const prefix$3 = !prefix_raw ? void 0 : actuallyCaseSensitive ? prefix_raw : prefix_raw.toLowerCase();
					const suffix$2 = !suffix_raw ? void 0 : actuallyCaseSensitive ? suffix_raw : suffix_raw.toLowerCase();
					const existingNode = node.dynamic?.find((s$1) => s$1.caseSensitive === actuallyCaseSensitive && s$1.prefix === prefix$3 && s$1.suffix === suffix$2);
					if (existingNode) nextNode = existingNode;
					else {
						const next = createDynamicNode(1, route.fullPath ?? route.from, actuallyCaseSensitive, prefix$3, suffix$2);
						nextNode = next;
						next.depth = depth;
						next.parent = node;
						node.dynamic ??= [];
				...
				case 3: {
					...
					const prefix$3 = !prefix_raw ? void 0 : actuallyCaseSensitive ? prefix_raw : prefix_raw.toLowerCase();
					const suffix$2 = !suffix_raw ? void 0 : actuallyCaseSensitive ? suffix_raw : suffix_raw.toLowerCase();
					const existingNode = node.optional?.find((s$1) => s$1.caseSensitive === actuallyCaseSensitive && s$1.prefix === prefix$3 && s$1.suffix === suffix$2);
					if (existingNode) nextNode = existingNode;
					else {
						const next = createDynamicNode(3, route.fullPath ?? route.from, actuallyCaseSensitive, prefix$3, suffix$2);
						nextNode = next;
						next.parent = node;
						next.depth = depth;
						node.optional ??= [];
				...
				case 2: {
					...
					const prefix$3 = !prefix_raw ? void 0 : actuallyCaseSensitive ? prefix_raw : prefix_raw.toLowerCase();
					const suffix$2 = !suffix_raw ? void 0 : actuallyCaseSensitive ? suffix_raw : suffix_raw.toLowerCase();
					const next = createDynamicNode(2, route.fullPath ?? route.from, actuallyCaseSensitive, prefix$3, suffix$2);
					nextNode = next;
					next.parent = node;
					next.depth = depth;
					node.wildcard ??= [];
...
function createDynamicNode(kind, fullPath, caseSensitive, prefix$3, suffix$2) {
	return {
		kind,
		depth: 0,
		static: null,
...
function findMatch(path, segmentTree, fuzzy = false) {
	...
	return {
		route: isFuzzyMatch ? leaf.node.notFound ?? leaf.node.route : leaf.node.route,
		params
	};
}
...
function getNodeMatch(path, parts, segmentTree, fuzzy) {
	...
	while (stack.length) {
		...
		if (fuzzy && node.notFound && isFrameMoreSpecific(bestFuzzy, frame)) bestFuzzy = frame;
		const isBeyondPath = index === partsLength;
		if (isBeyondPath) {
			if (node.route && (!pathIsIndex || node.isIndex)) {
				if (isFrameMoreSpecific(bestMatch, frame)) bestMatch = frame;
		...
		if (node.wildcard && isFrameMoreSpecific(wildcardMatch, frame)) for (const segment of node.wildcard) {
			const { prefix: prefix$3, suffix: suffix$2 } = segment;
			if (prefix$3) {
				if (isBeyondPath) continue;
				if (!(segment.caseSensitive ? part : lowerPart ??= part.toLowerCase()).startsWith(prefix$3)) continue;
			}
			if (suffix$2) {
				if (isBeyondPath) continue;
				const end = parts.slice(index).join("/").slice(-suffix$2.length);
				if ((segment.caseSensitive ? end : end.toLowerCase()) !== suffix$2) continue;
			}
			wildcardMatch = {
				node: segment,
				index,
		...
		if (node.optional) {
			...
			if (!isBeyondPath) for (let i = node.optional.length - 1; i >= 0; i--) {
				const segment = node.optional[i];
				const { prefix: prefix$3, suffix: suffix$2 } = segment;
				if (prefix$3 || suffix$2) {
					const casePart = segment.caseSensitive ? part : lowerPart ??= part.toLowerCase();
					if (prefix$3 && !casePart.startsWith(prefix$3)) continue;
					if (suffix$2 && !casePart.endsWith(suffix$2)) continue;
				}
				stack.push({
					node: segment,
					index: index + 1,
...
function resolvePath({ base, to, trailingSlash = "never", cache }) {
	...
	for (let i = 0; i < baseSegments.length; i++) {
		...
		const end = segment[5];
		const prefix$3 = part.substring(0, segment[1]);
		const suffix$2 = part.substring(segment[4], end);
		const value = part.substring(segment[2], segment[3]);
		if (kind === 1) joined += prefix$3 || suffix$2 ? `${prefix$3}{$${value}}${suffix$2}` : `$${value}`;
		else if (kind === 2) joined += prefix$3 || suffix$2 ? `${prefix$3}{$}${suffix$2}` : "$";
		else joined += `${prefix$3}{-$${value}}${suffix$2}`;
	}
...
function interpolatePath({ path, params, decodeCharMap }) {
	...
	while (cursor < length) {
		...
		if (kind === 2) {
			...
			usedParams["*"] = splat;
			const prefix$3 = path.substring(start, segment[1]);
			const suffix$2 = path.substring(segment[4], end);
			if (!splat) {
				isMissingParams = true;
				if (prefix$3 || suffix$2) joined += "/" + prefix$3 + suffix$2;
				continue;
			}
			const value = encodeParam("_splat", params, decodeCharMap);
			joined += "/" + prefix$3 + value + suffix$2;
			continue;
		}
		if (kind === 1) {
...
var executeBeforeLoad = (inner, matchId, index, route) => {
	...
	const preload$2 = resolvePreload(inner, matchId);
...
var loadRouteMatch = async (inner, index) => {
	...
	if (shouldSkipLoader(inner, matchId)) {
		...
		if (prevMatch._nonReactive.loaderPromise) {
			...
			const preload$2 = resolvePreload(inner, matchId);
			const staleAge = preload$2 ? route.options.preloadStaleTime ?? inner.router.options.defaultPreloadStaleTime ?? 3e4 : route.options.staleTime ?? inner.router.options.defaultStaleTime ?? 0;
			...
			const nextPreload = !!preload$2 && !inner.router.state.matches.some((d) => d.id === matchId);
			const match2 = inner.router.getMatch(matchId);
			...
			if (preload$2 && route.options.preload === false);
	...
	if (!route._componentsLoaded && route._componentsPromise === void 0) {
		const loadComponents = () => {
			...
			for (const type of componentTypes) {
				const preload$2 = route.options[type]?.preload;
				if (preload$2) preloads.push(preload$2());
			}
	...
	constructor(options) {
		...
		this.beforeLoad = () => {
			...
			this.__store.setState((s$1) => ({
				...
				location: this.latestLocation,
				pendingMatches,
				cachedMatches: s$1.cachedMatches.filter((d) => !pendingMatches.some((e) => e.id === d.id))
			}));
		};
		this.load = async (opts) => {
			...
			loadPromise = new Promise((resolve) => {
				this.startTransition(async () => {
					try {
							...
							onReady: async () => {
								this.startTransition(() => {
									this.startViewTransition(async () => {
										...
										batch(() => {
											this.__store.setState((s$1) => {
												const previousMatches = s$1.matches;
												const newMatches = s$1.pendingMatches || s$1.matches;
												...
												return {
													...
													cachedMatches: [...s$1.cachedMatches, ...exitingMatches.filter((d) => d.status !== "error" && d.status !== "notFound")]
												};
											});
						...
						this.__store.setState((s$1) => ({
							...s$1,
							statusCode: redirect2 ? redirect2.status : notFound$1 ? 404 : s$1.matches.some((d) => d.status === "error") ? 500 : 200,
							redirect: redirect2
						}));
					}
			...
			if (newStatusCode !== void 0) this.__store.setState((s$1) => ({
				...s$1,
		...
		this.updateMatch = (id, updater) => {
			this.startTransition(() => {
				...
				if (matchesKey) this.__store.setState((s$1) => ({
					...s$1,
					[matchesKey]: s$1[matchesKey]?.map((d) => d.id === id ? updater(d) : d)
				}));
			});
		};
		...
		this.invalidate = (opts) => {
			...
			this.__store.setState((s$1) => ({
				...s$1,
				matches: s$1.matches.map(invalidate),
				cachedMatches: s$1.cachedMatches.map(invalidate),
				pendingMatches: s$1.pendingMatches?.map(invalidate)
			}));
		...
		this.clearCache = (opts) => {
			...
			if (filter !== void 0) this.__store.setState((s$1) => {
				return {
					...s$1,
					cachedMatches: s$1.cachedMatches.filter((m) => !filter(m))
				};
			});
			else this.__store.setState((s$1) => {
				return {
					...s$1,
					cachedMatches: []
				};
		...
		this.preloadRoute = async (opts) => {
			...
			batch(() => {
				matches.forEach((match) => {
					if (!loadedMatchIds.has(match.id)) this.__store.setState((s$1) => ({
						...s$1,
						cachedMatches: [...s$1.cachedMatches, match]
					}));
				});
			});
			try {
...
var require_use_sync_external_store_shim_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var objectIs$3 = "function" === typeof Object.is ? Object.is : is$3, useState = React$3.useState, useEffect$1 = React$3.useEffect, useLayoutEffect$1 = React$3.useLayoutEffect, useDebugValue$1 = React$3.useDebugValue;
	...
	function checkIfSnapshotChanged(inst) {
		...
		try {
			...
			return !objectIs$3(inst, nextValue);
		} catch (error) {
			return !0;
		}
	}
	...
	exports.useSyncExternalStore = void 0 !== React$3.useSyncExternalStore ? React$3.useSyncExternalStore : shim$1;
}));
...
var require_with_selector_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var objectIs$2 = "function" === typeof Object.is ? Object.is : is$2, useSyncExternalStore = shim.useSyncExternalStore, useRef$1 = React$2.useRef, useEffect = React$2.useEffect, useMemo$2 = React$2.useMemo, useDebugValue = React$2.useDebugValue;
	exports.useSyncExternalStoreWithSelector = function(subscribe$1, getSnapshot, getServerSnapshot, selector, isEqual) {
		...
		instRef = useMemo$2(function() {
			function memoizedSelector(nextSnapshot) {
				...
				if (objectIs$2(memoizedSnapshot, nextSnapshot)) return currentSelection;
...
function useLoaderData(opts) {
	return useMatch({
		...
		structuralSharing: opts.structuralSharing,
		select: (s$1) => {
			return opts.select ? opts.select(s$1.loaderData) : s$1.loaderData;
		}
	});
}
function useLoaderDeps(opts) {
	...
	return useMatch({
		...rest,
		select: (s$1) => {
			return select ? select(s$1.loaderDeps) : s$1.loaderDeps;
		}
	});
}
...
function useLinkProps(options, forwardedRef) {
	...
	const currentSearch = useRouterState({
		select: (s$1) => s$1.location.search,
	...
	const preload$2 = options.reloadDocument || externalLink ? false : userPreload ?? router.options.defaultPreload;
	const preloadDelay = userPreloadDelay ?? router.options.defaultPreloadDelay ?? 0;
	const isActive = useRouterState({ select: (s$1) => {
		if (externalLink) return false;
		if (activeOptions?.exact) {
			if (!exactPathTest(s$1.location.pathname, next.pathname, router.basepath)) return false;
		} else {
			const currentPathSplit = removeTrailingSlash(s$1.location.pathname, router.basepath);
			const nextPathSplit = removeTrailingSlash(next.pathname, router.basepath);
			if (!(currentPathSplit.startsWith(nextPathSplit) && (currentPathSplit.length === nextPathSplit.length || currentPathSplit[nextPathSplit.length] === "/"))) return false;
		}
		if (activeOptions?.includeSearch ?? true) {
			if (!deepEqual(s$1.location.search, next.search, {
				partial: !activeOptions?.exact,
				ignoreUndefined: !activeOptions?.explicitUndefined
			})) return false;
		}
		if (activeOptions?.includeHash) return s$1.location.hash === next.hash;
		return true;
	} });
	...
	}, [doPreload]), intersectionObserverOptions, { disabled: !!disabled || !(preload$2 === "viewport") });
	import_react.useEffect(() => {
		if (hasRenderFetched.current) return;
		if (!disabled && preload$2 === "render") {
			doPreload();
	...
	const handleFocus = (_) => {
		if (disabled) return;
		if (preload$2) doPreload();
	};
	const handleTouchStart = handleFocus;
	const handleEnter = (e) => {
		if (disabled || !preload$2) return;
	...
	const handleLeave = (e) => {
		if (disabled || !preload$2 || !preloadDelay) return;
		const eventTarget = e.target;
...
function Transitioner() {
	...
	const { hasPendingMatches, isLoading } = useRouterState({
		select: (s$1) => ({
			isLoading: s$1.isLoading,
			hasPendingMatches: s$1.matches.some((d) => d.status === "pending")
		}),
		structuralSharing: true
	});
	...
	useLayoutEffect(() => {
		if (previousIsAnyPending && !isAnyPending) {
			...
			router.__store.setState((s$1) => ({
				...s$1,
				status: "idle",
				resolvedLocation: s$1.location
			}));
...
function CatchNotFound(props) {
	const resetKey = useRouterState({ select: (s$1) => `not-found-${s$1.location.pathname}-${s$1.status}` });
...
var Match = import_react.memo(function MatchImpl({ matchId }) {
	...
	const matchState = useRouterState({
		select: (s$1) => {
			const match = s$1.matches.find((d) => d.id === matchId);
	...
	const resetKey = useRouterState({ select: (s$1) => s$1.loadedAt });
	const parentRouteId = useRouterState({ select: (s$1) => {
		const index = s$1.matches.findIndex((d) => d.id === matchId);
		return s$1.matches[index - 1]?.routeId;
	} });
	...
	}), parentRouteId === "__root__" && router.options.scrollRestoration ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(OnRendered, {}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ScrollRestoration, {})] }) : null] });
});
function OnRendered() {
	const router = useRouter();
...
var MatchInner = import_react.memo(function MatchInnerImpl({ matchId }) {
	...
	const { match, key, routeId } = useRouterState({
		select: (s$1) => {
			const match2 = s$1.matches.find((d) => d.id === matchId);
...
var Outlet = import_react.memo(function OutletImpl() {
	...
	const routeId = useRouterState({ select: (s$1) => s$1.matches.find((d) => d.id === matchId)?.routeId });
	const route = router.routesById[routeId];
	const parentGlobalNotFound = useRouterState({ select: (s$1) => {
		const parentMatch = s$1.matches.find((d) => d.id === matchId);
		invariant(parentMatch, `Could not find parent match for matchId "${matchId}"`);
		return parentMatch.globalNotFound;
	} });
	const childMatchId = useRouterState({ select: (s$1) => {
		const matches = s$1.matches;
...
function MatchesInner() {
	const router = useRouter();
	const matchId = useRouterState({ select: (s$1) => {
		return s$1.matches[0]?.id;
	} });
	const resetKey = useRouterState({ select: (s$1) => s$1.loadedAt });
...
var useTags = () => {
	...
	const preloadLinks = useRouterState({
		select: (state) => {
			const preloadLinks2 = [];
			state.matches.map((match) => router.looseRoutesById[match.routeId]).forEach((route) => router.ssr?.manifest?.routes[route.id]?.preloads?.filter(Boolean).forEach((preload$2) => {
				preloadLinks2.push({
					tag: "link",
					attrs: {
						rel: "modulepreload",
...
var patternBodyEnd = /(<\/body>)/;
var patternHtmlEnd = /(<\/html>)/;
var patternClosingTag = /(<\/[a-zA-Z][\w:.-]*?>)/g;
...
async function readStream(stream$1, opts) {
	const reader = stream$1.getReader();
	try {
		let chunk;
		while (!(chunk = await reader.read()).done) opts.onData?.(chunk);
		opts.onEnd?.();
	} catch (error) {
		opts.onError?.(error);
	} finally {
...
function transformStreamWithRouter(router, appStream, opts) {
	...
	handleInjectedHtml();
	stopListeningToInjectedHtml = router.subscribe("onInjectedHtml", handleInjectedHtml);
	function handleInjectedHtml() {
		if (cleanedUp) return;
		router.serverSsr.injectedHtml.forEach((promise) => {
			processingCount++;
			promise.then((html) => {
				if (cleanedUp || finalPassThrough.destroyed) return;
				if (isAppRendering) routerStreamBuffer += html;
				else finalPassThrough.write(html);
			}).catch((err) => {
				injectedHtmlDonePromise.reject(err);
			}).finally(() => {
				processingCount--;
				if (!isAppRendering && processingCount === 0) injectedHtmlDonePromise.resolve();
			});
		});
	...
	readStream(appStream, {
		onData: (chunk) => {
			...
			if (bodyEndMatch && htmlEndMatch && bodyEndMatch.index < htmlEndMatch.index) {
				...
				finalPassThrough.write(chunkString.slice(0, bodyEndIndex) + getBufferedRouterStream() + leftoverHtml);
				leftover = "";
				leftoverHtml = "";
				return;
			}
			...
			while ((result = patternClosingTag.exec(chunkString)) !== null) lastIndex = result.index + result[0].length;
			if (lastIndex > 0) {
		...
		onEnd: () => {
			...
			if (processingCount === 0) injectedHtmlDonePromise.resolve();
			else {
				const timeoutMs = opts?.timeoutMs ?? 6e4;
				timeoutHandle = setTimeout(() => {
					injectedHtmlDonePromise.reject(/* @__PURE__ */ new Error("Injected HTML timeout after app render finished"));
				}, timeoutMs);
			}
		},
		onError: (error) => {
...
var require_react_dom_server_legacy_node_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function scriptReplacer$1(match, prefix$3, s$1, suffix$2) {
		return "" + prefix$3 + ("s" === s$1 ? "\\u0073" : "\\u0053") + suffix$2;
	}
	...
	function styleReplacer$1(match, prefix$3, s$1, suffix$2) {
		return "" + prefix$3 + ("s" === s$1 ? "\\73 " : "\\53 ") + suffix$2;
	}
	...
	function useActionState$1(action, initialState, permalink) {
		...
		if ("function" === typeof action.$$FORM_ACTION) {
			...
			"function" === typeof boundAction.$$FORM_ACTION && (action.$$FORM_ACTION = function(prefix$3) {
				...
				void 0 !== permalink && (permalink += "", prefix$3.action = permalink);
...
var require_react_dom_server_node_production = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function scriptReplacer(match, prefix$3, s$1, suffix$2) {
		return "" + prefix$3 + ("s" === s$1 ? "\\u0073" : "\\u0053") + suffix$2;
	}
	...
	function styleReplacer(match, prefix$3, s$1, suffix$2) {
		return "" + prefix$3 + ("s" === s$1 ? "\\73 " : "\\53 ") + suffix$2;
	}
...
var renderRouterToStream = async ({ request, router, responseHeaders, children }) => {
	if (typeof import_server_node.renderToReadableStream === "function") {
		const stream$1 = await import_server_node.renderToReadableStream(children, {
			signal: request.signal,
			nonce: router.options.ssr?.nonce,
			progressiveChunkSize: Number.POSITIVE_INFINITY
		});
		if (isbot(request.headers.get("User-Agent"))) await stream$1.allReady;
		const responseStream = transformReadableStreamWithRouter(router, stream$1);
...
export { require_jsx_runtime as S, isResolvedRedirect as _, RouterProvider as a, invariant as b, lazyRouteComponent as c, useNavigate as d, useSearch as f, isRedirect as g, executeRewriteInput as h, HeadContent as i, createFileRoute as l, require_with_selector as m, TSR_SCRIPT_BARRIER_ID as n, createRouter as o, useRouter as p, Scripts as r, Outlet as s, renderRouterToStream as t, createRootRoute as u, rootRouteId as v, createControlledPromise as x, isNotFound as y };
