import { i as __require, t as __commonJSMin } from "../../_rolldown.mjs";
...
var require_token_io = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var import_token_error$2 = require_token_error();
	function findRootDir() {
		try {
			...
			throw new import_token_error$2.VercelOidcTokenError("Token refresh only supported in node server environments");
		}
		return null;
	}
...
var require_auth_config = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var import_token_util$1 = require_token_util();
	function getAuthConfigPath() {
		const dataDir = (0, import_token_util$1.getVercelDataDir)();
...
var require_token_util = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = __toCommonJS$1(token_util_exports);
	...
	async function getVercelOidcToken(authToken, projectId, teamId) {
		...
		if (!res.ok) throw new import_token_error$1.VercelOidcTokenError(`Failed to refresh OIDC token: ${res.statusText}`);
	...
	function findProjectInfo() {
		...
		if (!dir) throw new import_token_error$1.VercelOidcTokenError("Unable to find project root directory. Have you linked your project with `vc link?`");
		const prjPath = path.join(dir, ".vercel", "project.json");
		if (!fs.existsSync(prjPath)) throw new import_token_error$1.VercelOidcTokenError("project.json not found, have you linked your project with `vc link?`");
	...
	function saveToken(token, projectId) {
		...
		if (!dir) throw new import_token_error$1.VercelOidcTokenError("Unable to find user data directory. Please reach out to Vercel support.");
	...
	function getTokenPayload(token) {
		...
		if (tokenParts.length !== 3) throw new import_token_error$1.VercelOidcTokenError("Invalid token. Please run `vc env pull` and try again");
