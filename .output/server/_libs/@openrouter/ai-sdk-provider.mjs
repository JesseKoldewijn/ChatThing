import { $ as unknown, B as array, G as literal, I as _enum, J as object, Q as union, Y as record, Z as string, q as number, z as any } from "../@ai-sdk/gateway.mjs";
		...
		case "user": {
			...
			const contentParts = content.map((part) => {
				...
				switch (part.type) {
					...
					case "file": {
						if ((_b2 = part.mediaType) == null ? void 0 : _b2.startsWith("image/")) return {
							type: "image_url",
		...
		case "assistant": {
			...
			for (const part of content) switch (part.type) {
				...
				case "reasoning": {
					...
					if (parsedPartProviderOptions.success && ((_e = (_d = parsedPartProviderOptions.data) == null ? void 0 : _d.openrouter) == null ? void 0 : _e.reasoning_details)) accumulatedReasoningDetails.push(...parsedPartProviderOptions.data.openrouter.reasoning_details);
					break;
				}
				case "file": break;
				default: break;
...
var OpenRouterChatLanguageModel = class {
	...
	async doGenerate(options) {
		...
		const fileAnnotations = (_k = choice.message.annotations) == null ? void 0 : _k.filter((a) => a.type === "file");
		const hasToolCalls = choice.message.tool_calls && choice.message.tool_calls.length > 0;
		const hasEncryptedReasoning = reasoningDetails.some((d) => d.type === "reasoning.encrypted" && d.data);
		return {
			content,
			finishReason: hasToolCalls && hasEncryptedReasoning && choice.finish_reason === "stop" ? "tool-calls" : mapOpenRouterFinishReason(choice.finish_reason),
			usage: usageInfo,
			warnings: [],
			providerMetadata: { openrouter: OpenRouterProviderMetadataSchema.parse({
				...
				}, ((_s = (_r = response.usage) == null ? void 0 : _r.prompt_tokens_details) == null ? void 0 : _s.cached_tokens) != null ? { promptTokensDetails: { cachedTokens: response.usage.prompt_tokens_details.cached_tokens } } : {}), ((_u = (_t = response.usage) == null ? void 0 : _t.completion_tokens_details) == null ? void 0 : _u.reasoning_tokens) != null ? { completionTokensDetails: { reasoningTokens: response.usage.completion_tokens_details.reasoning_tokens } } : {}), ((_w = (_v = response.usage) == null ? void 0 : _v.cost_details) == null ? void 0 : _w.upstream_inference_cost) != null ? { costDetails: { upstreamInferenceCost: response.usage.cost_details.upstream_inference_cost } } : {})
			}) },
			request: { body: args },
			response: {
				id: response.id,
		...
		return {
			stream: response.pipeThrough(new TransformStream({
				transform(chunk, controller) {
					...
					if (delta.tool_calls != null) for (const toolCallDelta of delta.tool_calls) {
						...
						if (toolCalls[index] == null) {
							...
							if (((_e = toolCallDelta.function) == null ? void 0 : _e.name) == null) throw new InvalidResponseDataError({
								data: toolCallDelta,
				...
				flush(controller) {
					...
					if (hasToolCalls && hasEncryptedReasoning && finishReason === "stop") finishReason = "tool-calls";
...
var OpenRouterEmbeddingModel = class {
	...
	async doEmbed(options) {
		...
		return {
			...
			providerMetadata: ((_a15 = responseValue.usage) == null ? void 0 : _a15.cost) ? { openrouter: { usage: { cost: responseValue.usage.cost } } } : void 0,
			response: {
				headers: responseHeaders,
				body: responseValue
			}
...
function withUserAgentSuffix(headers, ...userAgentSuffixParts) {
	...
	const newUserAgent = [cleanedHeaders["user-agent"] || "", ...userAgentSuffixParts].filter(Boolean).join(" ");
...
function createOpenRouter(options = {}) {
	...
	const getHeaders = () => withUserAgentSuffix(__spreadValues(__spreadValues({ Authorization: `Bearer ${loadApiKey({
		apiKey: options.apiKey,
		environmentVariableName: "OPENROUTER_API_KEY",
		description: "OpenRouter"
	})}` }, options.headers), options.api_keys && Object.keys(options.api_keys).length > 0 && { "X-Provider-API-Keys": JSON.stringify(options.api_keys) }), `ai-sdk/openrouter/${VERSION}`);
